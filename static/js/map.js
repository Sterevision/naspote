(function () {
    ...
    function initMap() {
        ...
        map.on('click', function (event) {
            // Task 1: если только что сработал long tap, обычный клик не обрабатываем
            if (legendLongTapFired) {
                legendLongTapFired = false;
                return;
            }

            // Task 1: если легенда открыта, тап по карте закрывает её
    ...
    function hasCommonInterests(spot) {
        ...
        return myInt.some(function (i) {
            return theirInt.indexOf(i) !== -1;
        });
    }
    ...
    function initAudioRecorder() {
        ...
        if (!recordBtn || !statusEl || !audioInput) return;
    ...
    function resetAddForm() {
        ...
        if (recBtn) {
            recBtn.textContent = '🎙️ Записать атмосферу (10 сек)';
        ...
        try {
            ...
            box.innerHTML = comments.map(function (comment) {
                ...
                if (comment.created_at) {
                    ...
                    if (!isNaN(d.getTime())) {
                        date = d.toLocaleDateString([], { day: '2-digit', month: '2-digit' }) +
                            ' ' +
                ...
                return '<div class="comment-row">' +
                    ...
                    (comment.user && comment.user.avatar_url
                        ? '<img src="' + esc(safeUrl(comment.user.avatar_url)) + '" alt="">'
                        : esc(initial)) +
                    '</div>' +
                    '<div class="info">' +
    ...
    function bindUI() {
        ...
        if (addSpotOverlay) {
            addSpotOverlay.addEventListener('click', function (event) {
                if (event.target === addSpotOverlay) closeAddSheet();
            });
        }
        ...
        if (spotSheetOverlay) {
            spotSheetOverlay.addEventListener('click', function (event) {
                if (event.target === spotSheetOverlay) closeSpotSheet();
            });
        }
        ...
        if (ownToggle) {
            ownToggle.addEventListener('click', function () {
                ...
                var ownCount = allSpots.filter(function (s) {
    ...
    // Task 1: легенда открывается долгим тапом по карте
    function initHiddenLegend() {
        var legendPanel = $('legendPanel');
        if (!legendPanel || !map) return;
        ...
        function isOpen() {
            return legendPanel.classList.contains('open');
        }
        ...
        function cancelLongTap() {
            if (legendLongTapTimer) {
                clearTimeout(legendLongTapTimer);
                legendLongTapTimer = null;
            }
        }

        function releaseLongTap() {
            cancelLongTap();
        ...
        function startLongTap(x, y) {
            ...
            legendLongTapTimer = setTimeout(function () {
                legendLongTapFired = true;
                setOpen(!isOpen());
        ...
        function moveLongTap(x, y) {
            if (!legendLongTapTimer || startX === null || startY === null) return;
        ...
        mapEl.addEventListener('touchstart', function (event) {
            if (event.touches && event.touches.length === 1) {
                startLongTap(event.touches[0].clientX, event.touches[0].clientY);
            }
        }, { passive: true });

        mapEl.addEventListener('touchmove', function (event) {
            if (event.touches && event.touches.length === 1) {
                moveLongTap(event.touches[0].clientX, event.touches[0].clientY);
            }
        }, { passive: true });

        mapEl.addEventListener('touchend', releaseLongTap, { passive: true });
        mapEl.addEventListener('touchcancel', releaseLongTap, { passive: true });

        mapEl.addEventListener('mousedown', function (event) {
            startLongTap(event.clientX, event.clientY);
        });

        mapEl.addEventListener('mousemove', function (event) {
            moveLongTap(event.clientX, event.clientY);
        });

        mapEl.addEventListener('mouseup', releaseLongTap);
        mapEl.addEventListener('mouseleave', releaseLongTap);
