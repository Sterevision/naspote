...
def register():
    ...
    if not email or not password or not username:
        flash("Заполните email, имя пользователя и пароль"); return redirect(url_for("register"))
...
def feed_view():
    ...
    # friend_ids уже содержит только друзей + самого пользователя — RLS и так не пустит чужое,
    # поэтому доп. фильтр по visibility здесь был лишним и скрывал метки друзей "только для друзей"
...
def verify_telegram_init_data(init_data: str, bot_token: str) -> dict | None:
    """Проверяет HMAC-подпись Telegram initData. Возвращает user dict или None."""
    if not bot_token:
        return None
...
def api_telegram_auth():
    """Бесшовный вход через Telegram Mini App."""
    ...
    existing = sb.table("profiles").select("id, username").eq("telegram_id", tg_id).execute()
    ...
    if existing.data:
        ...
        fake_password = f"tg_{tg_id}_{uuid.uuid4().hex[:12]}"

        try:
            auth_res = sb.auth.sign_up({"email": fake_email, "password": fake_password})
        except Exception as e:
            # Возможно, email уже занят (редкий кейс)
            return jsonify({"ok": False, "error": str(e)}), 400

        if not auth_res.user:
            return jsonify({"ok": False, "error": "user creation failed"}), 400
        ...
        if auth_res.session:
            ...
            try:
                ...
                # Если insert не удался (RLS), пробуем через обычный клиент
                try:
                    sb.table("profiles").insert(profile_data).execute()
                except Exception as e:
                    return jsonify({"ok": False, "error": f"profile: {e}"}), 400
    ...
    # Если пользователь найден, но нужно восстановить сессию —
    # делаем sign_in с сгенерированным паролем (не идеально, но работает для MVP)
    # В продакшне лучше использовать Supabase Custom JWT или magic link
    session["user_id"] = uid
    return jsonify({"ok": True, "username": uname})
...
# --- Соцдоказательство: сколько друзей/людей отметились у того же заведения сегодня ---
...
def api_spot_social_proof(spot_id):
    ...
    if not org_id:
        return jsonify({"friends_count": 0, "total_today": 0, "friends": []})
...
def api_spot_collaborate(spot_id):
    ...
    uid = session["user_id"]
    try:
        sb.table("spot_collaborators").insert({"spot_id": spot_id, "user_id": uid}).execute()
    except Exception:
        pass  # уже присоединился
    return jsonify({"ok": True})
...
def api_search_organizations():
    ...
    if lat is not None and lng is not None:
        orgs.sort(key=lambda o: 999999 if o.get("lat") is None else (o["lat"] - lat) ** 2 + (o["lng"] - lng) ** 2)
        orgs = orgs[:8]
    return jsonify(orgs[:15] if lat is None else orgs)
