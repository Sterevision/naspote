...
CATEGORIES = ["Бар", "Клуб", "Кофейня", "Ресторан", "Коворкинг",
              "Караоке", "Спорт", "Вечеринка", "Природа",
              "Выставка/галерея", "Другое"]
...
def login_required(view):
    ...
    def wrapped(*args, **kwargs):
        # ИСПРАВЛЕНО: разделённые return вместо кортежа с тернарником
...
def map_view():
    ...
    if not result.data:
        session.clear()
        flash("Профиль не найден. Войдите заново.")
        return redirect(url_for("login"))
...
def friends_view():
    ...
    profile = result.data[0] if result.data else {}
    return render_template("friends.html", incoming=incoming.data, accepted=accepted.data,
                           my_id=uid, profile=profile)
...
def settings_view():
    ...
    if cover and cover.filename:
        update_data["cover_url"] = upload_to_bucket(sb, "avatars", uid, cover)
    try:
        sb.table("profiles").update(update_data).eq("id", uid).execute()
...
def api_spots_create():
    ...
    is_live = request.form.get("is_live", "true") == "true"
    ...
    data = {"owner_id": uid, "title": title, "description": description,
            "lat": float(lat), "lng": float(lng),
            "visibility": visibility if visibility in ("public", "friends") else "public",
            "is_live": is_live, "placement_type": placement_type, "expires_at": expires_at}
    if photo_url:
    ...
    if request.form.get("wave_enabled") == "true":
        ...
        if wave_max and wave_max.isdigit():
            data["wave_max_people"] = int(wave_max)
...
def api_achievements():
    sb = get_supabase(session["access_token"], session.get("refresh_token"))
    uid = session["user_id"]
    prof = sb.table("profiles").select("xp, level").eq("id", uid).execute()
    profile = prof.data[0] if prof.data else {}
    badges = sb.table("user_achievements").select("*").eq("user_id", uid).order("earned_at", desc=True).execute()
    return jsonify({"achievements": badges.data or [], "xp": profile.get("xp", 0), "level": profile.get("level", 1)})
...
def api_spot_social_proof(spot_id):
    sb = get_supabase(session["access_token"], session.get("refresh_token"))
    uid = session["user_id"]
    spot_res = sb.table("spots").select("organization_id").eq("id", spot_id).execute()
    spot = spot_res.data[0] if spot_res.data else {}
    org_id = spot.get("organization_id")
    if not org_id:
...
def api_spot_collaborate(spot_id):
    sb = get_supabase(session["access_token"], session.get("refresh_token"))
    try:
        sb.table("spot_collaborators").insert({"spot_id": spot_id, "user_id": session["user_id"]}).execute()
    except Exception:
        pass
    return jsonify({"ok": True})
...
def api_spot_voice():
    ...
    url = upload_to_bucket(sb, "voice-notes", session["user_id"], voice)
