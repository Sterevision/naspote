...
CATEGORIES = [
    "Бар", "Клуб", "Кофейня", "Ресторан", "Коворкинг",
...
def login_required(view):
    ...
    def wrapped(*args, **kwargs):
        # Проверка Bearer токена из Telegram (если передан в заголовке)
        ...
        # Стандартная проверка сессии Flask
...
def profile_view(username):
    ...
    if not is_me:
        f = sb.table("friendships").select("*").or_(
            f"and(requester_id.eq.{session['user_id']},addressee_id.eq.{profile['id']}),and(requester_id.eq.{profile['id']},addressee_id.eq.{session['user_id']})"
        ).execute()
        if f.data:
...
def api_telegram_auth():
    ...
    # Новый случайный пароль генерируется при каждом входе — используется только
    # чтобы получить сессию через admin.update_user_by_id + sign_in_with_password ниже.
    ...
    if existing.data:
        ...
        if not auth_res.user:
            return jsonify({"ok": False, "error": "creation failed"}), 400
        ...
        profile_data = {
            "id": uid,
            "username": uname,
            "display_name": tg_first_name + (" "+tg_last_name if tg_last_name else ""),
            "account_type": "person",
            "telegram_id": tg_id
        }
        if tg_photo_url:
            profile_data["avatar_url"] = tg_photo_url
    ...
    try:
        ...
        if not sign_in_res.session:
            return jsonify({"ok": False, "error": "session failed"}), 400
